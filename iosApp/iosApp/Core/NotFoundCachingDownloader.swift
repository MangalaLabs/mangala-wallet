...
class NotFoundCachingDownloader: ImageDownloader {
    ...
    func noFaviconsFound(forDomain domain: String) {
        guard let hashedKey = FaviconsHelperCore.defaultResource(forDomain: domain, sourcesProvider: sourcesProvider)?.cacheKey else { return }
    ...
    func shouldDownload(forDomain domain: String, referenceDate: Date = Date()) -> Bool {
        guard let hashedKey = FaviconsHelperCore.defaultResource(forDomain: domain, sourcesProvider: sourcesProvider)?.cacheKey else { return false }
