...
//    @IBOutlet weak var logoText: UIImageView!
    ...
    func startSession(){
        if(self.address.isEmpty == false && sessionsProvider == nil){
            ...
            currentTab?.sessionsProvider = sessionsProvider
            print("1991 sessionsProvider start")
        }
...
//        startOnboardingFlowIfNotSeenBefore()
    ...
    private func configureTabManager() {
        ...
        print("2000 address1 configureTabManager \(address) ")
...
//        currentTab?.onShareAction(forLink: link, fromView: omniBar.shareButton, orginatedFromMenu: false)
    ...
    private func openSwitchNetwork(){
        let viewController = SwitchNetworkController()
...
extension MainViewController: TabSwitcherButtonDelegate {
    ...
    func launchNewTab(_ button: TabSwitcherButton) {
        newTab()
        print("1997 launchNewTab")
    }

    func showTabSwitcher(_ button: TabSwitcherButton) {
        Pixel.fire(pixel: .tabBarTabSwitcherPressed)
        showTabSwitcher()
        print("1997 showTabSwitcher")
    }

    func showTabSwitcher() {
        print("1997 showTabSwitcher 2")
        if let currentTab = currentTab {
...
extension MainViewController: SwitchNetworkControllerDelegate{
    
    func chainIdCallback(callbackId: Int){
        print("2022 callbackId \(callbackId)")
        currentTab?.switchChain(newChainId: callbackId)
    }
}
