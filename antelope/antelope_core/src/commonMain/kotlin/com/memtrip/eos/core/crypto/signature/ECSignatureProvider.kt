...
import com.mangala.wallet.utils.ext.testBit
...
class ECSignatureProvider private constructor(
    ...
    private val hashAsBigInteger: BigInteger = BigInteger.fromByteArray( hash, Sign.POSITIVE)
) {
    ...
    private fun validComponentLength(components: Array<BigInteger>): Boolean {
        return components[0].toTwosComplementByteArray().size == LENGTH && components[1].toTwosComplementByteArray().size == LENGTH
    }
    ...
    private fun deterministicGenerateK(
        ...
        if (nonce > 0) {
            val nonceByteArray = BigInteger.fromLong(nonce.toLong()).toByteArray()
        ...
        return createSignature(BigInteger.fromByteArray( v, Sign.POSITIVE), v, k, keyCurve, hashAsBigInteger, privateKey)
    }
    ...
    private fun createSignature(
        ...
        if (k1.signum() <= 0 || k1 >= keyCurve.n() || !valid(q, r, s)) {
            ...
            createSignature(BigInteger.fromByteArray(v, Sign.POSITIVE), v, k, keyCurve, hashAsBigInteger, privateKey)
        }
    ...
    private fun multiply(p: ECPoint, k: BigInteger): ECPoint {
        val h = k.multiply(BigInteger.fromInt(3))
    ...
    companion object {
        ...
        private fun recoverPubKey(keyCurve: SecP256K1KeyCurve, messageSigned: ByteArray, signature: ECKey.ECDSASignature, recId: Int): EosPublicKey? {
            ...
            PreconditionsG.checkArgument(signature.r >= BigInteger.ZERO, "r must be positive")
            PreconditionsG.checkArgument(signature.s >= BigInteger.ZERO, "s must be positive")
            PreconditionsG.checkNotNull(messageSigned)
            ...
            val e = BigInteger.fromByteArray( messageSigned, Sign.POSITIVE)
            ...
            q = curve.createPoint(q.getNewX()?.toBigInteger(), q.getNewY()?.toBigInteger(), true)
