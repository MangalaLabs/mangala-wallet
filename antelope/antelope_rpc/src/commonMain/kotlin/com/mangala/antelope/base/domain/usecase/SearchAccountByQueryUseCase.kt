...
class SearchAccountByQueryUseCase(private val repository: AntelopeRepository) {

    suspend operator fun invoke(
        blockchainType: BlockchainType,
        request: GetTableByScopeRequest
    ): Result<GetTableByScopeResponse> {
        return when (val result = repository.getTableByScope(blockchainType, request)) {
            is ApiResponse.Success -> Result.success(result.body)
            else -> {
                if (result is ApiResponse.Error.CancellationError) {
                    Result.failure(kotlin.coroutines.cancellation.CancellationException())
                } else {
                    Result.failure(Exception("Failed to get table by scope"))
                }
            }
        }
    }
