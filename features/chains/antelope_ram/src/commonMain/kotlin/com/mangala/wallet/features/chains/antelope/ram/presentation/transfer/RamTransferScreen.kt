...
class RamTransferScreen(
    private val accountName: String
) : BaseScreen<RamTransferScreenModel>(), KoinComponent {

    override val screenName: String = MangalaAnalytics.Screens.ANTELOPE_RAM_TRANSFER
    override val screenClassName: String = RamTransferScreen::class.simpleName.orEmpty()

    private val scanQRCode: ScanQRCode by inject()

    @Composable
    override fun createScreenModel(): RamTransferScreenModel {
    ...
    @Composable
    override fun ScreenContent(screenModel: RamTransferScreenModel) {
        ...
        MangalaBottomSheetNavigator(
            ...
            LaunchedEffect((uiState as? RamTransferUiState.Success)?.uiModel?.showPinPrompt) {
                if ((uiState as? RamTransferUiState.Success)?.uiModel?.showPinPrompt == true) {
                    bottomSheetNavigator.show(pinScreen)
                    screenModel.onPinPromptShown()
                }
            }

            if ((uiState as? RamTransferUiState.Success)?.uiModel?.resourceRequiredBreakdown != null) {
                val uiModel = (uiState as? RamTransferUiState.Success)?.uiModel
            ...
            when (uiState) {
                is RamTransferUiState.ExecuteRamTransferSuccess -> {
                    ExecuteTransactionSuccess(
                        onClickBack = { navigator.pop() },
                        textTitle = MR.strings.message_transfer_ram_success.desc().localized()
                    ) {
                        MangalaGradientButton(
                            label = MR.strings.button_transfer_ram_continue.desc().localized(),
    ...
    @OptIn(ExperimentalMaterialApi::class)
    @Composable
    fun RamTransferScreen(
        uiState: RamTransferUiState,
        onBackClicked: () -> Unit,
        ...
        val uiModel = remember(uiState) { (uiState as? RamTransferUiState.Success)?.uiModel }
        ...
        MaxSizeBox(
            ...
            MaxSizeColumn(
                ...
                MangalaTopBarTitleInMiddle(
                    isLoading = false,
                    titleTopBar = MR.strings.title_transfer_ram.desc().localized(),
                    ...
                    MaxSizeColumn(
                        ...
                        MaxWidthColumn(
                            ...
                            RamInformation(
                                ...
                                totalRam = "${uiModel.ramAvailableString} kb",
    ...
    fun InputSectionTransfer(
        ...
        ItemInput(
            ...
            hint = MR.strings.hint_transfer_ram_enter_account_name.desc().localized(),
            validationStatus = uiModel.recipientAccountNameValidationStatus,
            imeAction = ImeAction.Next,
            invalidContent = {
                TextDescription2(
                    text = MR.strings.label_transfer_ram_invalid_account_name.desc().localized(),
                    fontSize = FontType.TINY,
                    fontWeight = FontWeight.Normal,
                    color = MaterialTheme.mangalaColors.buttonDestructiveContainer,
                )
            ...
            titleContent = {
                TextDescription2(
                    text = MR.strings.label_transfer_ram_recipient.desc().localized(),
                    modifier = Modifier.fillMaxWidth(),
        ...
        ItemInput(
            ...
            titleContent = {
                TextDescription2(
                    text = MR.strings.label_transfer_ram_memo.desc().localized(),
                    modifier = Modifier.fillMaxWidth(),
                    fontSize = FontType.SMALL,
                    fontWeight = FontWeight.Normal,
                    color = MaterialTheme.mangalaColors.textPrimary
