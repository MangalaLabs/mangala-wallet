...
object ContractMethodHelper {
    fun getMethodId(methodSignature: String): ByteArray {
        return CryptoUtils.sha3(methodSignature.encodeToByteArray()).copyOfRange(0, 4)
    }
        ...
        arguments.forEach { argument ->
            when (argument) {
                ...
                is List<*> -> {
                    ...
                    data += pad(unsignedBigIntergerToByteArray((BigInteger.fromLong(arguments.size * 32L + arraysData.size))))
                    arraysData += encode(addresses)
                }
                is ByteArray -> {
                    data += pad(unsignedBigIntergerToByteArray(BigInteger.fromLong(arguments.size * 32L + arraysData.size)))
                    arraysData += pad(BigInteger.fromLong(argument.size.toLong()).toByteArray()) + argument
                }
            }
        }
    ...
    private fun encode(array: List<Address>): ByteArray {
        var data = pad(unsignedBigIntergerToByteArray(BigInteger.fromLong(array.size.toLong())))
