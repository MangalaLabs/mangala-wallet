...
object RLEncoder4Final {
    ...
    fun encode(rawTransaction: RawTransaction): ByteArray {
        ...
        val encodedFields = listOf(
            encodeBigInteger(rawTransaction.nonce),
            encodeBigInteger(rawTransaction.gasPrice),
            encodeBigInteger(rawTransaction.gasLimit),
            encodeByteArray(rawTransaction.recipient.hexStringToByteArray()),
            encodeBigInteger(rawTransaction.value.toBigInteger()),
            encodeByteArray(rawTransaction.data)
        )

        // RLP-encode the list of fields
        val encodedList = encodeList(encodedFields)

        // Combine the RLP-encoded transaction prefix and the RLP-encoded list of fields
        return encodeByteArray(encodedList.size.toBigInteger().toByteArray()) + encodedList
    }
    ...
    private fun encodeByteArray(bytes: ByteArray): ByteArray {
        return when {
            ...
            bytes.size < 56 -> {
                ...
                val lengthBytes = encodeByteArray(
                    BigInteger.parseString(bytes.size.toLong().toString()).toByteArray()
                )
    ...
    fun String.hexStringToByteArray(): ByteArray {
        val cleanedHexString = this.replace("0x", "")
        return if (cleanedHexString.isEmpty()) {
            ByteArray(0)
        } else {
            cleanedHexString.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
        }
    }
    ...
    fun encodeTransaction(rawTransaction: RawTransaction): ByteArray {
        val encodedFields = listOf(
            encodeBigInteger(rawTransaction.nonce),
            encodeBigInteger(rawTransaction.gasPrice),
            encodeBigInteger(rawTransaction.gasLimit),
            encodeAddress(rawTransaction.recipient),
            encodeBigInteger(rawTransaction.value.toBigInteger()),
            encodeByteArray(rawTransaction.data)
        )
